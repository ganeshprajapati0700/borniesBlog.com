        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="border border-slate-200 rounded-lg overflow-hidden shadow-sm bg-slate-50">
                ...
                <div id="seo-section" class="p-4 space-y-6 hidden">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-input-field label="Meta Title" name="meta_title" placeholder="SEO Title" value="{{ old('meta_title', $post->meta_title) }}" />
                        <x-input-field label="Meta Keywords" name="meta_keywords" placeholder="Keywords (comma separated)" value="{{ old('meta_keywords', $post->meta_keywords) }}" />
                    </div>
                    <div>
                        <label for="meta_description" class="block text-sm font-medium text-slate-700 mb-2">Meta Description</label>
                        <textarea name="meta_description" id="meta_description" rows="2" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors">{{ old('meta_description', $post->meta_description) }}</textarea>
                    </div>
                    <hr class="border-slate-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-input-field label="OG Title" name="og_title" placeholder="Open Graph Title" value="{{ old('og_title', $post->og_title) }}" />
                        <x-input-field label="OG Image URL" name="og_image" placeholder="Social share image URL" value="{{ old('og_image', $post->og_image) }}" />
                    </div>
                    <div>
                        <label for="og_description" class="block text-sm font-medium text-slate-700 mb-2">OG Description</label>
                        <textarea name="og_description" id="og_description" rows="2" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors">{{ old('og_description', $post->og_description) }}</textarea>
                    </div>
                </div>
            </div>
